...
namespace Celebrity.Forms.EdicionViews
{
    public partial class FrmCategoriasList : FrmBase
    {
        ...
        private IEnumerable<Category> categorias;
        private IEnumerable<Category> GetCategorias
        {
            get
            {
        ...
        public FrmCategoriasList(ICategoryRepository repository)
        {
            InitializeComponent();
        ...
        private void BtnAddCategorias_Click(object sender, EventArgs e)
        {
            using (var f = new FrmCategoria(repository.GetComboItemsCategorias().ToList()))
            {
                if (f.ShowDialog() == DialogResult.OK)
                {
                    ...
                    repository.InsertarCategoria(Category.Create(f.Nombre, parent == null ? (Guid?)null : new Guid(parent)));
                    LoadFromDataBase();
                    Fill();
                }
            }
        ...
        private void OnDoubleClick(object sender, EventArgs e)
        {
            ...
            {
                var categoria = Lvw.SelectedItems[0].Tag as Category;
                using (var f = new FrmCategoria(repository.GetComboItemsCategorias().ToList()))
                {
                    f.Id = categoria.Id;
