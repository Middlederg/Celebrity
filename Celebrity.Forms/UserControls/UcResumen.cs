...
namespace Celebrity.Forms.UserControls
{
    public partial class UcResumen : UcBase
    {
        ...
        public void UpdateTable(IEnumerable<Equipo> equipos, IEnumerable<Ronda> rondas)
        {
            ...
            foreach (var ronda in rondas)
            {
                ...
                foreach (var equipo in equipos)
                {
                    var lblDetalle = CreateCell(equipo.NumeroAciertos(ronda.NumeroRonda).ToString(), ContentAlignment.MiddleRight, ancho);
